using AutoMapper;
using Bodegas.Domain.Entities;
using Bodegas.Domain.Helpers;
using Bodegas.Domain.Ports;
using Bodegas.Domain.Services;
using MediatR;

namespace Bodegas.Applications.BodegasService.Commands
{
    public class ActualizarBodegaAliadoCommandHandler : AsyncRequestHandler<ActualizarBodegaAliadoCommand>
    {
        private readonly IMapper _mapper;
        private readonly IBodegaServices _bodegaServices;
        public ActualizarBodegaAliadoCommandHandler(IMapper mapper, IBodegaServices bodegaServices)
        {
            _mapper = mapper;
            _bodegaServices = bodegaServices;
        }

        protected override async Task Handle(ActualizarBodegaAliadoCommand request, CancellationToken cancellationToken)
        {
            Bodega bodega = _mapper.Map<Bodega>(request);
            await _bodegaServices.UpdateAsync(bodega);
        }
    }
}
